...
const TopChartCard = ({ song, i, isPlaying, activeSong, handlePauseClick, handlePlayClick }) => (
  <div className="w-full flex flex-row items-center hover:bg-gray-400 bg-gray-200 bg-opacity-80 shadow-md shadow-gray-300 py-2 p-4 rounded-lg cursor-pointer mb-2">
    ...
    <div ref={divRef} className='xl:ml-6 ml-0 xl:mb-0 mb-6 flex-1 xl:max-w-[500px] max-w-full flex flex-col'>
      <div className="w-full flex flex-col">
        <div className="flex flex-row justify-between items-center">
          <h2 className="text-black font-bold text-2xl">Top Charts</h2>
          <Link to='/top-charts'>
      ...
      <div className="w-full flex flex-col mt-8">
        ...
        <Swiper slidesPerView='auto' spaceBetween={15} freeMode centeredSlides centeredSlidesBounds modules={[FreeMode]} className='mt-4 bg-gray-200 rounded-full'>
          {topPlays?.map((song, i) => (
            <SwiperSlide key={song?.key} style={{ width: '25%', height: 'auto' }} className='rounded-full animate-slider-right'>
              <Link to={`/artists/${song?.artists[0].adamid}`}>
